...
def test_reads_anthropic_api_key_from_injected_source() -> None:
    settings = load_settings(_DictSource({"VELORA_ANTHROPIC_API_KEY": "sk-ant-test-value"}))

    assert settings.anthropic_api_key == "sk-ant-test-value"


def test_default_source_falls_back_to_none_anthropic_api_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("VELORA_ANTHROPIC_API_KEY", raising=False)
...
def test_reads_elevenlabs_api_key_from_injected_source() -> None:
    settings = load_settings(_DictSource({"VELORA_ELEVENLABS_API_KEY": "el-test-value"}))

    assert settings.elevenlabs_api_key == "el-test-value"


def test_default_source_falls_back_to_none_elevenlabs_api_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("VELORA_ELEVENLABS_API_KEY", raising=False)
